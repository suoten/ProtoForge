...
def main():
    ...
    run_parser.add_argument("--host", default="0.0.0.0", help="监听地址 (默认: 0.0.0.0)")
    run_parser.add_argument("--port", type=int, default=8000, help="监听端口 (默认: 8000)")
    ...
    demo_parser.add_argument("--port", type=int, default=8000, help="监听端口")
...
def _run_server(host="0.0.0.0", port=8000, reload=False, log_level="info", demo_mode=False):
    ...
    if host == "0.0.0.0" and settings.host != "0.0.0.0":
        host = settings.host
    if port == 8000 and settings.port != 8000:
        port = settings.port
