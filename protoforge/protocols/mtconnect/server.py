...
class MtConnectServer(ProtocolServer):
    ...
    def _build_probe(self) -> str:
        ...
        for dev_id, config in self._device_configs.items():
            ...
            devices_xml.append(
                f'  <Device id="{escape(dev_id)}" name="{escape(config.name)}" uuid="{self._device_uuid}">\n'
                f'    <Description manufacturer="ProtoForge">Simulated Device</Description>\n'
                f'    <DataItems>\n'
                + "\n".join(data_items) + "\n"
                f'    </DataItems>\n'
                f'  </Device>'
    ...
    def _build_current(self) -> str:
        ...
        for dev_id, behavior in self._behaviors.items():
            ...
            for point in config.points:
                ...
                self._sequence += 1
            streams_xml.append(
                f'  <DeviceStream name="{escape(config.name)}" uuid="{self._device_uuid}">\n'
    ...
    async def create_device(self, device_config: DeviceConfig) -> str:
        ...
        logger.info("MTConnect device created: %s", device_config.id)
