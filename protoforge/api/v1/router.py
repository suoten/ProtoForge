...
async def start_protocol(protocol_name: str, config: dict[str, Any] = None):
    ...
    try:
        ...
        log_bus.emit(protocol_name, "system", "", "protocol_start",
                     f"Protocol {protocol_name} started", config)
        return {"status": "ok"}
...
async def quick_create_device(params: dict[str, Any]):
    ...
    device_id = params.get("id", device_name.lower().replace(" ", "-").replace("（", "(").replace("）", ")"))
    ...
    config = DeviceConfig(
        ...
        points=template.points, protocol_config=template.protocol_config,
    )
    engine = _get_engine()
    db = _get_database()
    log_bus = _get_log_bus()
...
async def get_logs(
    ...
    log_bus = _get_log_bus()
    return log_bus.get_recent(count=count, protocol=protocol, device_id=device_id)
...
async def quick_test(scope: str = "all", target_id: Optional[str] = None):
    ...
    if scope == "all" or scope == "device":
        for dev_id, dev in engine._devices.items():
            ...
            writable_points = [p for p in dev.config.points if p.access != "r"]
            if writable_points:
