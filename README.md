...
- 修复协议服务器（OPC UA/S7/MC/HTTP）使用 `asyncio.create_task()` 后台启动，端口绑定失败时 API 仍返回 200 OK 的问题。现在 `start_protocol()` 会等待 0.3 秒检查服务器状态，如果立即进入 ERROR 状态则返回 503 错误
- 添加协议启动时的配置日志，方便排查端口配置问题

**协议管理界面修复：**

- 修复协议服务页面缺少"全部停止"按钮，只有"全部启动"无法批量停止协议
- 添加 `stopAll` 函数和 `stoppingAll` 状态，支持一键停止所有运行中的协议

**i18n 插值修复：**

- 修复确认对话框中 `{n}` 未替换为实际数字的问题（如"将启动 {n} 个协议"显示为原始模板而非"将启动 3 个协议"），统一改为 `{count}` 并正确传参

**健康检查修复：**

- 修复 Dashboard 健康检查中"数据库: 操作失败"/"Engine: 操作失败"的显示问题，改为更准确的"异常"标签

**设备恢复修复：**

- 修复 `create_device()` 已存在设备时抛出 `ValueError` 导致启动恢复设备失败的问题，增加 `allow_update` 参数用于恢复场景
